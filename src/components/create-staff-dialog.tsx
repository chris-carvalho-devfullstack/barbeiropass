...
import { Loader2, Plus, Scissors, UserPlus, CheckCircle2, XCircle, Camera, Trash } from "lucide-react";
...
export function CreateStaffDialog({ onStaffCreated }: CreateStaffDialogProps) {
  ...
  useEffect(() => {
    ...
    async function loadInitialData() {
      try {
        ...
        if (memberData) {
          setBarbershopId(memberData.barbershop_id);
          form.setValue("barbershop_id", memberData.barbershop_id, { shouldValidate: true });
        }
  ...
  const onSubmit: SubmitHandler<StaffFormValues> = async (data) => {
    ...
    setLoading(true);
    const toastId = toast.loading("A registrar profissional...");
    ...
    try {
      ...
      const response = await fetch("/api/staff", {
        method: "POST",
        body: formData, // fetch define os headers do multipart automaticamente!
      });

      const result = await response.json();

      if (!response.ok) throw new Error(result.error || "Ocorreu um erro ao registrar o membro.");

      toast.success(`Profissional registrado com sucesso!`, { id: toastId });
      form.reset();
      setIsOpen(false);
      onStaffCreated(); 
    } catch (error: unknown) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto w-full p-6">
        <DialogHeader className="mb-4">
          <DialogTitle className="text-2xl font-black">Registrar Profissional</DialogTitle>
          <DialogDescription>
