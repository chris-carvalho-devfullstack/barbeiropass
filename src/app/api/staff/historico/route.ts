...
// Tipagem local para o mapeamento
interface OrderRow {
  id: string;
  created_at: string;
...
export async function GET(req: Request) {
  try {
    ...
    let query = supabase
      .from("pos_orders")
      .select(`
        id, created_at, customer_name, source_type, payment_method, total_amount,
        pos_order_items!inner(id, barber_id, item_type)
      `, { count: 'exact' })
      .eq("pos_order_items.barber_id", staffId)
    ...
    // Filtrar IDs duplicados de forma estrita
    const uniqueOrdersMap = new Map<string, OrderRow>();
    if (data) {
      data.forEach((item: unknown) => {
        const order = item as OrderRow;
    ...
    return NextResponse.json({ data: Array.from(uniqueOrdersMap.values()), count });
  } catch (error: unknown) {
