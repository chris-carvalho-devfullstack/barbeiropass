...
import { 
  Users, Loader2, Briefcase, ShieldCheck, MoreHorizontal, 
...
// Interface atualizada para incluir a URL do avatar
...
export default function EquipePage() {
  ...
  async function fetchStaff() {
    try {
      setLoading(true);
      // Atualizado para buscar o avatar_url no banco de dados
  ...
  async function handleDelete(id: string) {
    if (!confirm("Tem certeza absoluta? Esta ação excluirá o profissional permanentemente.")) return;
    ...
    <div className="flex flex-col gap-6 p-2 md:p-6 animate-in fade-in zoom-in duration-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">
              Gestão de plantel, comissões e acessos.
            </p>
          </div>
        </div>
                ...
                {filteredAndSortedStaff.length === 0 ? (
                  ...
                  filteredAndSortedStaff.map((member) => (
                    <TableRow key={member.id} className="hover:bg-blue-50/30 dark:hover:bg-zinc-900/50 transition-colors group">
                      ...
                      >
                        {/* LÓGICA DE EXIBIÇÃO DA FOTO NA TABELA */}
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 shadow-xl rounded-xl">
                            ...
                            <DropdownMenuItem onClick={() => handleDelete(member.id)} className="text-red-600 focus:text-red-600 cursor-pointer">
      ...
      <Dialog open={!!selectedStaff} onOpenChange={(open) => !open && setSelectedStaff(null)}>
        <DialogContent className="sm:max-w-md p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
          {selectedStaff && (
            <>
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 flex items-center gap-4 text-white relative">
                
                {/* LÓGICA DE EXIBIÇÃO DA FOTO NO MODAL */}
                <div className="size-16 rounded-full bg-white/20 border-2 border-white flex items-center justify-center text-2xl font-black shadow-inner overflow-hidden shrink-0">
                ...
                <div>
                  <h3 className="text-xl font-bold truncate">{selectedStaff.full_name}</h3>
                  <span className="text-blue-100 font-medium text-sm flex items-center gap-1 opacity-90">
                    Código ID: <span className="font-mono bg-black/20 px-1.5 py-0.5 rounded text-xs">{selectedStaff.unique_code}</span>
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6 bg-white dark:bg-zinc-950">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 dark:bg-zinc-900 p-3 rounded-xl border border-slate-100 dark:border-zinc-800">
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Cargo</p>
                    <Badge variant="outline" className={`${roleMap[selectedStaff.role].color} shadow-sm border-0`}>
                  ...
                  <div className="bg-slate-50 dark:bg-zinc-900 p-3 rounded-xl border border-slate-100 dark:border-zinc-800 col-span-2">
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Contrato Financeiro</p>
                ...
                <Button 
                  className="w-full font-bold h-12 text-md shadow-md bg-slate-900 hover:bg-slate-800 text-white transition-all"
                  onClick={() => router.push(`/equipe/${selectedStaff.id}`)}
                >
                  Acessar Ficha Completa
                  <ChevronRight className="size-5 ml-2 text-slate-400" />
                </Button>
              </div>
            </>
          )}
